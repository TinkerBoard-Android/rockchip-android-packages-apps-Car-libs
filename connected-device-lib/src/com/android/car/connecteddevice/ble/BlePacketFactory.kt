...
private fun getTotalPacketNumber(messageId: Int, payloadSize: Int, maxSize: Int): Int {
    val headerSizeWithoutTotalPackets =
        FIXED_32_SIZE + FIELD_NUMBER_ENCODING_SIZE +
            getEncodedSize(messageId) + FIELD_NUMBER_ENCODING_SIZE +
            getEncodedSize(min(payloadSize, maxSize)) + FIELD_NUMBER_ENCODING_SIZE
    for (value in 1..PACKET_NUMBER_ENCODING_SIZE) {
        val packetHeaderSize =
            headerSizeWithoutTotalPackets + value +
    ...
    loge(
        TAG,
        "Cannot get valid total packet number for message: messageId: $messageId," +
...
private fun getEncodedSize(value: Int): Int {
    return when {
        ...
        value and (0.inv() shl 7) == 0 -> {
            loge(TAG, "Get a negative value from proto")
            1
        }
